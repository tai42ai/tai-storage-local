...
``tai42_storage_local`` is on the allowlist. The rule (see the README): the shipped
package imports
the async filesystem driver (aiofiles), the shared platform substrate
(tai42-contract + tai42-kit, pydantic-settings)
...
The runtime walk imports ``tai42_storage_local`` and every submodule in a fresh
subprocess, then inspects ``sys.modules``. Running it in a subprocess that
imports ONLY ``tai42_storage_local`` means the assertion covers the SHIPPED package's
...
# Every third-party root the shipped ``tai42_storage_local`` graph pulls in -- the
