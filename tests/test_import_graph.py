...
``tai_storage_local`` is on the allowlist. The rule (see the README): the shipped
package imports
the async filesystem driver (aiofiles), the shared platform substrate
(tai-contract + tai-kit, pydantic-settings)
...
The runtime walk imports ``tai_storage_local`` and every submodule in a fresh
subprocess, then inspects ``sys.modules``. Running it in a subprocess that
imports ONLY ``tai_storage_local`` means the assertion covers the SHIPPED package's
...
PACKAGE = "tai_storage_local"
ALLOWED_FIRST_PARTY = frozenset({PACKAGE, "tai_contract", "tai_kit"})

# Every third-party root the shipped ``tai_storage_local`` graph pulls in -- the
...
def test_shipped_package_imports_only_allowlisted_roots() -> None:
    ...
    assert result.returncode == 0, (
        f"importing the shipped tai_storage_local graph failed:\nstdout:\n{result.stdout}\nstderr:\n{result.stderr}"
    )

    offenders = [line for line in result.stdout.splitlines() if line]
    assert offenders == [], f"non-allowlisted roots in the tai_storage_local module graph: {offenders}"
...
def test_shipped_sources_name_only_allowlisted_roots() -> None:
    ...
    assert offenders == {}, f"non-allowlisted import roots in the tai_storage_local sources: {offenders}"
